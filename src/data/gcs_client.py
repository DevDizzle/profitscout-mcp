...
class GCSClient:
    ...
    def _read_json_blob(self, blob_path: str) -> Optional[Dict[str, Any]]:
    ...
    def _read_text_blob(self, blob_path: str) -> Optional[str]:
    ...
    async def get_news_analysis(
        ...
        if as_of == "latest":
            blob_path = self._get_latest_file_from_prefix(
                "headline-news/", ticker, ".json"
            )
        else:
        ...
        if not blob_path:
            return {
                "ticker": ticker.upper(),
                "message": "No news analysis found."
            }
    ...
    async def get_fundamental_analysis(
        ...
        if as_of == "latest":
            blob_path = self._get_latest_file_from_prefix(
                "fundamentals-analysis/", ticker, ".json"
            )
        else:
    ...
    async def get_financial_analysis(
        ...
        if as_of == "latest":
            blob_path = self._get_latest_file_from_prefix(
                "financials-analysis/", ticker, ".json"
            )
        else:
        ...
        if not blob_path:
            return {
                "ticker": ticker.upper(),
                "message": "No financial analysis found."
            }
    ...
    async def get_business_summary(
        ...
        if as_of == "latest":
            blob_path = self._get_latest_file_from_prefix(
                "business-summaries/", ticker, ".json"
            )
        else:
    ...
    async def get_macro_thesis(self, as_of: str = "latest") -> Optional[Dict[str, Any]]:
    ...
    async def get_mda_analysis(
        self, ticker: str, as_of: str = "latest"
    ) -> Optional[Dict[str, Any]]:
        """Get MD&A analysis for a ticker."""
        if as_of == "latest":
            blob_path = self._get_latest_file_from_prefix(
                "mda-analysis/", ticker, ".json"
            )
        else:
        ...
        if not blob_path:
            return {
                "ticker": ticker.upper(),
                "message": "No MD&A analysis found."
            }
    ...
    async def get_transcript_analysis(
        ...
        if as_of == "latest":
            blob_path = self._get_latest_file_from_prefix(
                "transcript-analysis/", ticker, ".json"
            )
        else:
        ...
        if not blob_path:
            return {
                "ticker": ticker.upper(),
                "message": "No transcript analysis found."
            }
