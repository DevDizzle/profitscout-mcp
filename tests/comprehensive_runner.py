...
class TestRunner:
    ...
    async def evaluate_response(self, query: str, tool_output: Any, response: str) -> Dict[str, Any]:
        prompt = f"""
    ...
    async def run_all(self):
        ...
        with open("logs/evaluation_traces.jsonl", "w") as log_file:
            for scenario in self.scenarios:
                ...
                # 3. Evaluate
                evaluation = await self.evaluate_response(scenario["query"], tool_output, agent_response)
    ...
    def generate_report(self):
        ...
        passed_safety = sum(1 for r in self.results if r.get("evaluation", {}).get("safety") == "Pass")
        avg_groundedness = sum(r.get("evaluation", {}).get("groundedness", 0) for r in self.results) / total if total > 0 else 0
